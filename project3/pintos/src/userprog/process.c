...
process_execute (const char *file_name)
{
  ...
  char new[128];
  strlcpy (new, file_name, PGSIZE);
  char *save_ptr;
  char *input = strtok_r(new, " ", &save_ptr);/* Create a new thread to execute FILE_NAME. */
...
start_process (void *file_name_)
{
  ...
  bool success;
    ...
    strlcpy (new, file_name, PGSIZE);
    char *save_ptr;
    char *input = strtok_r(new, " ", &save_ptr);
  /* Initialize interrupt frame and load executable. */
  memset (&if_, 0, sizeof if_);
  if_.gs = if_.fs = if_.es = if_.ds = if_.ss = SEL_UDSEG;
  if_.cs = SEL_UCSEG;
  if_.eflags = FLAG_IF | FLAG_MBS;
  success = load (input, &if_.eip, &if_.esp);
    if(success){
        char *save_ptr;
        char *args;
        char *argv[128];
...
{
    struct thread *cur = thread_current ();
    tid_t *tid_list = cur->tid_list;
    int i = 0;
    for(i = 0; i<128; i++){
        if(tid_list[i] == 0) continue;
        if(tid_list[i] == child_tid){
            struct thread *child = thread_by_tid(child_tid);
            if(child == NULL) return -1;
            sema_down(&(child->thread_S));
            tid_list[i] = 0;
...
process_exit (void)
{
  ...
  uint32_t *pd;
    int mapping;
    for(mapping = 0; mapping<128; mapping++){
        struct mmap_elem *mmap_E = cur->mmap_list[mapping];
        if(mmap_E){
            struct page *page = malloc(sizeof(struct page));
            page->upage = mmap_E->upage;
            while(1){
                struct hash_elem *file_page_elem = hash_find(&(cur->page_hash), &(page->page_elem));
                if(file_page_elem == NULL) break;
                struct page *file_page = hash_entry(file_page_elem, struct page, page_elem);
                if(pagedir_is_dirty(cur->pagedir, file_page->upage)){
                    file_write_at(file_page->file, file_page->upage, file_page->read_bytes, file_page->ofs);
                }
                page->upage += PGSIZE;
            }
            file_close(mmap_E->file);
            free(cur->mmap_list[mapping]);
            cur->mmap_list[mapping] = 0;
        }
    }
  /* Destroy the current process's page directory and switch back
     to the kernel-only page directory. */
    sema_up(&(cur->thread_S));
